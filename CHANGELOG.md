...
- **Skill activation is exempt from the 10k tool-output spill.** A SKILL.md
  ...
  result now passes through whole up to 60,000 characters (a per-tool band
  list on the upstream `ToolOutputLimits`, `SKILL_PASSTHROUGH_CHARS`) and
  spills only beyond that. `read_skill_file` keeps the general threshold.
