...
- `hooks/` — Claude-Code-compatible lifecycle hook engine (session/prompt/tool/
  compaction events). Observe-only except SessionStart/UserPromptSubmit (inject context) and PreCompact (may block a *manual* /compact via exit 2 or `{"decision":"block"}`; block verdicts on auto compaction are logged and ignored). Project-local hooks run only when trusted (`MARIM_TRUST_PROJECT_HOOKS`).
