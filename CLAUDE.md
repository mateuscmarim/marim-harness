...
(see `.env.example`): `MARIM_PROVIDER` (`openrouter`|`local`|`google`|`claude-cli`|`openai-codex`|`zen`|`zen-go`), `MARIM_MODEL`,
...
(the id is checkpointed in the spawn's sidecar meta). `openai-codex` runs
native Pydantic AI agents on a Codex subscription, reusing `codex login`
credentials through upstream OAuth (`config/codex_subscription.py`; the
account's visible model catalog in `config/codex_subscription_catalog.py`,
MCP schema-reference normalization in `config/codex_schema.py`) — marim owns
its tools, permissions, MCP, sub-agents and sessions there, exactly as for
any API provider. The former `codex-cli` executor (app-server, collab
sub-agents, `backend: codex-cli` spawns) is retired: a saved `codex-cli`
selection or an interrupted `codex-cli` child fails with migration guidance
(`config/retired.py`) instead of silently falling back, while old transcripts
stay readable.
...
- `subagents/` — `runner.py` (`SubagentRunner`: spawn-lifecycle coordinator),
  ...
  `cli_spawn.py` (`backend: claude-cli` execute/resume orchestration: builds the spawn's approval broker), `masking.py`
