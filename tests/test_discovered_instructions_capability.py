...
class _FakeMcp:
    ...
    def discovered_server_instructions(self, discovered):
        return [(s, t) for (s, t) in self._pairs if any(d.startswith(s + "_") for d in discovered)]
...
async def test_only_uninjected_servers_added():
    ...
    seeded = _ReqCtx(
        _instruction_messages("mddocs", "g") + [ModelRequest(parts=[UserPromptPart("hi")])]
    )
    ...
    assert (
        sum(
            ...
            for p in m.parts
            if "«mcp-guidance:mddocs»" in getattr(p, "content", "")
        )
        == 1
    )
