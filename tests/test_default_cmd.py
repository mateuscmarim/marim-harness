...
def test_claim_target_notices_a_stale_daemon_claim(tmp_path):
    """The daemon that held the session is gone (its claim file outlived its
    lock). The launch takes the session over LOCALLY — which is right — but
    must say so: a user who ran ``marim --session <id>`` to attach otherwise
    gets a local session whose only tell is a status bar without ``daemon``."""
    import io
    ...
    session_path = _write_session(tmp_path, "20260101-000000-aaaaaa")
    dead = try_acquire(session_path, kind="daemon", endpoint="http://127.0.0.1:8643")
    ...
    try:
        ...
        if claim is not None:
            claim.release()
    text = err.getvalue()
    assert "stale" in text and "locally" in text and "not attached" in text
    assert "http://127.0.0.1:8643" in text and "20260101-000000-aaaaaa" in text


def test_claim_target_is_silent_after_a_previous_local_run(tmp_path):
    """A TUI or headless run that exited is the normal predecessor — no notice."""
    import io
    ...
    session_path = _write_session(tmp_path, "20260101-000000-aaaaaa")
    previous = try_acquire(session_path, kind="tui")
...
def test_stale_daemon_notice_is_none_without_a_daemon_predecessor():
    from marim_harness.interfaces.cli.default_cmd import _stale_daemon_notice
    from marim_harness.session.claim import Holder, SessionClaim

    assert _stale_daemon_notice(None, "sid") is None
    assert _stale_daemon_notice(SessionClaim(Path("x.claim"), None), None) is None
    assert _stale_daemon_notice(SessionClaim(Path("x.claim"), None), "sid") is None
    tui = SessionClaim(Path("x.claim"), None, Holder(pid=1, kind="tui", endpoint=None))
    assert _stale_daemon_notice(tui, "sid") is None
    daemon = SessionClaim(Path("x.claim"), None, Holder(pid=7, kind="daemon", endpoint=None))
    notice = _stale_daemon_notice(daemon, "sid")
    assert notice is not None and "pid 7" in notice and " at " not in notice
...
def test_start_tui_hands_the_stale_daemon_notice_to_the_app(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(headless_mod, "run_headless", _must_not_be_awaited)
    err = io.StringIO()
...
def test_run_default_refuses_before_building_a_claimed_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(default_cmd, "_tui_available", lambda: True)
    monkeypatch.setattr(default_cmd, "_launch_tui", lambda h, **kw: built.append("tui"))
