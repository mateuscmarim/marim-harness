...
def test_list_form():
    ...
    ps = parse_lsp_providers(block, bundled=False, source="global", plugin_root=None, strict=True)
...
def test_backend_rejected_for_third_party_strict():
    ...
    with pytest.raises(LspProviderError):
        parse_lsp_providers(block, bundled=False, source="global", plugin_root=None, strict=True)
...
def test_backend_allowed_for_bundled():
    ...
    (p,) = parse_lsp_providers(block, bundled=True, source="bundled", plugin_root=None, strict=True)
...
def test_command_and_backend_mutually_exclusive():
    ...
    with pytest.raises(LspProviderError):
        parse_lsp_providers(block, bundled=True, source="bundled", plugin_root=None, strict=True)
...
def _prov(
    language, exts, *, command=None, backend=None, probe=None, diagnostics="lsp", source="global"
):
...
def test_registry_availability_probe_present(monkeypatch):
    ...
    monkeypatch.setattr("marim_harness.lsp.registry.shutil.which", lambda b: "/usr/bin/gopls")
    assert reg.availability("go").available is True


def test_registry_availability_probe_missing():
    reg = LspRegistry([_prov("go", [".go"], command="gopls", probe=["definitely-not-on-path-xyz"])])
