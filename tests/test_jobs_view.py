...
def test_job_to_dto_enriches_agent_meta():
    dto = job_to_dto(
        _agent("job-1", "done", "2026-07-23T00:05:00+00:00"),
...
def test_assemble_reads_meta_only_for_agent_stream_ids():
    ...
    bash = Job(
        id="job-6", kind="bash", label="ls", status="done", finished_at="2026-07-23T00:02:00+00:00"
    )
    assemble(
        [bash, _agent("job-7", "done", "2026-07-23T00:03:00+00:00")], history=[], read_meta=reader
    )
...
def test_detail_dto_carries_prompt_and_result():
    dto = detail_dto(
        _agent("job-8", "done", "2026-07-23T00:04:00+00:00"), result="the full output", meta=None
    )
